...
func mapRepositoryImport(payload map[string]interface{}, id string) (cloudevents.Event, cee.ValidationError) {
	event := cloudevents.NewEvent(Version)

	event.SetID(id)
	event.SetSource(lookupValueFromPayloadPath(payload, "repository", "owner", "url"))
	event.SetType("com.github.repository_import." + lookupValueFromPayloadPath(payload, "action"))
