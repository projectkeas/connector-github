...
func mapInstallationRepositories(payload map[string]interface{}, id string) (cloudevents.Event, cee.ValidationError) {
	event := cloudevents.NewEvent(Version)

	event.SetID(id)
	event.SetSource(lookupValueFromPayloadPath(payload, "installation", "account", "url"))
	event.SetType("com.github.installation_repository." + lookupValueFromPayloadPath(payload, "action"))
