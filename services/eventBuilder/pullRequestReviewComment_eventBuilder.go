...
func mapPullRequestReviewComment(payload map[string]interface{}, id string) (cloudevents.Event, cee.ValidationError) {
	event := cloudevents.NewEvent(Version)

	event.SetID(id)
	event.SetSource(lookupValueFromPayloadPath(payload, "pull_request", "url"))
	event.SetType("com.github.pull_request_review_comment." + lookupValueFromPayloadPath(payload, "action"))
	event.SetSubject(lookupValueFromPayloadPath(payload, "comment", "id"))
	event.SetTime(parseTime(lookupValueFromPayloadPath(payload, "pull_request", "updated_at")))
